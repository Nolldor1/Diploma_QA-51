...
1. **Оплата картой:**
    **Открыть** [страницу](https://localhost:8080/) => кликнуть "Купить" (оплата картой) => в открывшемся окне в доступных полях ввести валидные данные => кликнуть "Продолжить";  
    `Результат:`
...
2. **Оплата в кредит:**
    **Открыть** [страницу](https://localhost:8080/) => кликнуть "Купить в кредит" => в открывшемся окне в доступных полях ввести валидные данные => кликнуть "Продолжить";  
      `Результат:`
...
2. **Незаполненное поле "Номер карты"/неверный ввод данных в поле "Номер карты":**
    ...
    2.3. **Открыть** [страницу](https://localhost:8080/) => кликнуть "Купить" (оплата картой) => в поле "Номер карты" **ввести не валидный номер** (4444 4444 4444 4440), остальные поля заполнить валидными данными => кликнуть  "Продолжить";  
